...
namespace SimpleWeather.NET.Controls.Graphs
{
    public sealed partial class LineView : BaseGraphScrollView<LineViewData, LineDataSeries, LineGraphEntry>
    {
        ...
        {
            return new LineViewGraph(ScrollViewer);
        }
        
        internal override LineViewGraph Graph => (LineViewGraph)base.Graph;
        
        public bool DrawGridLines { set => Graph.DrawGridLines = value; }
        public bool DrawDotLine { set => Graph.DrawDotLine = value; }
        public bool DrawDotPoints { set => Graph.DrawDotPoints = value; }
        public bool DrawGraphBackground { set => Graph.DrawGraphBackground = value; }
        public bool DrawSeriesLabels { set => Graph.DrawSeriesLabels = value; }
        public Color BackgroundLineColor { set => Graph.BackgroundLineColor = value; }
        ...
        internal sealed partial class LineViewGraph : BaseGraphView<LineViewData, LineDataSeries, LineGraphEntry>
        {
            ...
            private float GraphHeight
            {
                get
                {
                    ...
                    if (DrawIconLabels) graphHeight -= (float)IconSize;
            ...
            private float GraphBottom
            {
                get
                {
                    ...
                    if (DrawIconLabels)
                        graphBottom -= (IconSize + iconBottomMargin);
                    else
    ...
    #endif
                {
                    (obj as LineViewGraph)?.UpdateBackgroundLineColor();
                }
            }
            ...
            {
                if (yCoordinateList == null) return;
            ...
            private void RefreshDrawDotList()
            {
                if (!IsDataEmpty)
                {
                    ...
                    for (int k = 0; k < Data.DataCount; k++)
                    {
                        int drawDotSize = drawDotLists[k].IsEmpty() ? 0 : drawDotLists[k].Count;

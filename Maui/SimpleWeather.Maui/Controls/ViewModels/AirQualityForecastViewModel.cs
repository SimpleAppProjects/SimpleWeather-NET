...
using ResAQI = SimpleWeather.Resources.Strings.AQIndex;
            ...
            enumerable?.ForEach(aqi =>
            {
                if (aqi.index.HasValue)
                {
                    if (aqiIndexData == null)
                    {
                        aqiIndexData = new()
                        {
                            GraphLabel = ResStrings.label_airquality
                        };
                    }

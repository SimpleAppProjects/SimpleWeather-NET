...
namespace SimpleWeather.Controls
{
    public class UVIndexViewModel
    {
        ...
        public UVIndexViewModel(UV uvIdx)
        {
            ...
            if (uvIdx.index < 3)
            {
                Description = SimpleLibrary.ResLoader.GetString("/UVIndex/UV_0");
                ProgressColor = Colors.LimeGreen;
            }
            else if (uvIdx.index < 6)
            {
                Description = SimpleLibrary.ResLoader.GetString("/UVIndex/UV_3");
                ProgressColor = Colors.Yellow;
            }
            else if (uvIdx.index < 8)
            {
                Description = SimpleLibrary.ResLoader.GetString("/UVIndex/UV_6");
                ProgressColor = Colors.Orange;
            }
            else if (uvIdx.index < 11)
            {
                Description = SimpleLibrary.ResLoader.GetString("/UVIndex/UV_8");
                ProgressColor = Color.FromArgb(0xFF, 0xBD, 0x00, 0x35); // Maroon
            }
            else if (uvIdx.index >= 11)
            {
                Description = SimpleLibrary.ResLoader.GetString("/UVIndex/UV_11");

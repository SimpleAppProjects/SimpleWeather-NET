...
using SimpleWeather.Weather_API.Google;
...
namespace SimpleWeather.Weather_API.WeatherData
{
    public abstract partial class WeatherProviderImpl : IWeatherProvider, IRateLimitedRequest
    {
        ...
        public async Task<Weather> GetWeather(LocData location)
        {
            if (location == null || location.query == null)
                throw new WeatherException(WeatherUtils.ErrorStatus.Unknown,
                    new ArgumentNullException("location?.query"));
            ...
            if (weather.condition.pollen == null)
            {
                await this.RunCatching(async () =>
                {
                    ...
                    if (SettingsManager.DevSettingsEnabled && RemoteConfigService.IsProviderEnabled(WAPI.AccuWeather) &&
                        weather.source != WAPI.AccuWeather)
                    {
        ...
        {
            var aqicnData = await new AQICNProvider().GetAirQualityData(location);
                ...
                try
                {
                    if (aqicnData?.uvi_forecast?.Count > 0)
                    {
                        for (int i = 0; i < aqicnData.uvi_forecast.Count; i++)
                        {
                            ...
                            var date = DateTime.ParseExact(uviData.day, "yyyy-MM-dd", CultureInfo.InvariantCulture,
                            ...
                            if (weather.condition.uv == null && date.Equals(weather.condition.observation_time.Date))
                            {
                                if (weather.astronomy.sunrise != null && weather.astronomy.sunset != null)
                                {
                                    ...
                                    if (obsLocalTime < weather.astronomy.sunrise.TimeOfDay ||
                                        obsLocalTime > weather.astronomy.sunset.TimeOfDay ||
                                        Math.Abs((weather.astronomy.sunrise.TimeOfDay - obsLocalTime).TotalHours) <=
                                        2 || Math.Abs((weather.astronomy.sunset.TimeOfDay - obsLocalTime).TotalHours) <=
                                        2)
                                    {
        ...
        public virtual Task<ICollection<WeatherAlert>> GetAlerts(LocData location)
        {
            ...
            {
                return new WeatherApiProvider().GetAlerts(location);
            }
        }
        ...
        public abstract Task<string> UpdateLocationQuery(LocData location);

        public virtual string LocaleToLangCode(string iso, string name)
        {
            return "EN";
        }
